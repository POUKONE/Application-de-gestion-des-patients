...
# ANCIEN CODE (CAUSE DE L'ERREUR)
def connect_to_db():
    # Ici, le nom de la base de données est incorrect (avec .sql)
    return mysc.connect(host="localhost", user="root", password="", database="hopital")

# Compter les patients pour générer le matricule
# Compter les patients pour générer le matricule
def count_patient():
    conn = None  # Initialisation à None
    cur = None   # Initialisation à None
    try:
        conn = connect_to_db()
        cur = conn.cursor()
    ...
    except Exception:  # On utilise 'Exception' ou mysc.Error pour capturer l'erreur
        return "24SJI1"
    finally:
        if cur is not None:  # On ferme seulement si le curseur a été créé
            cur.close()
        if conn is not None: # On ferme seulement si la connexion a été établie
            conn.close()

# Note: Il est recommandé de capturer mysc.Error ou Exception spécifique au lieu de 'except:' seul.
...
def actualiser_liste_patients():
    ...
    conn = None  # <-- NOUVEAU : Initialisation à None
    cur = None   # <-- NOUVEAU : Initialisation à None
    ...
    try:
        ...
        # Note: Cette ligne affichera désormais les vraies erreurs de connexion !
        print("Erreur lors du chargement (Connexion/SQL) :", e) 
    finally:
        if cur is not None:  # <-- NOUVEAU : Fermeture conditionnelle
            cur.close()
        if conn is not None: # <-- NOUVEAU : Fermeture conditionnelle
            conn.close()

# Note : N'oubliez pas d'appliquer la même correction à la fonction `count_patient()` 
# et aux autres fonctions CRUD (`ajouter`, `supprimer`) qui utilisent un bloc try/finally.

# Interface
root = Tk()
root.title("Gestion des patients - Saint Jean Hôpital")
root.configure(bg="#434197")

matricule_var = StringVar(value=count_patient())
modifier_mode = [False]

titre = Label(root, text="APPLICATION DE GESTION DES PATIENTS", font=("Helvetica", 18, "bold"), bg="#643888", fg="#2c073d")
titre.grid(row=0, column=0, columnspan=2, pady=20)

form_frame = Frame(root, bg='#643888')
form_frame.grid(row=1, column=0, padx=20, sticky="n")

Label(form_frame, text="Matricule:", bg='#643888').grid(row=0, column=0, sticky="w")
...
for i, lab in enumerate(labels):
    Label(form_frame, text=f"{lab}:", bg='#643888').grid(row=i+1, column=0, sticky="w")
    e = Entry(form_frame)
...
Button(form_frame, text="Enregistrer", command=ajouter, bg="#a08eaf", fg='white').grid(row=7, column=0, pady=10)
Button(form_frame, text="Modifier", command=modifier, bg='#a08eaf', fg='white').grid(row=7, column=1)
Button(form_frame, text="Supprimer", command=supprimer, bg='#a08eaf', fg='white').grid(row=8, column=0, columnspan=2, pady=5)

# Affichage liste patients
liste_frame = Frame(root, bg="#0f0e0f")
liste_frame.grid(row=1, column=1, padx=10)

Label(liste_frame, text="Liste des patients", font=("Helvetica", 14, "bold"), bg="#11a0a5", fg='#c2185b').pack(pady=5)
